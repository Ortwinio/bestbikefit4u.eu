You have task 044: Add missing structural sections to all guides.

IMPORTANT: You are at 14% context. Spawn a subagent to do the heavy implementation work.

Your job:
1. Read plans/guide-guideline-alignment/01-missing-structural-sections.md (the full prompt)
2. Launch it as a subagent: codex "# Prompt 01 — Add Missing Structural Sections to All Guides
...
3. Monitor and report back when done

The subagent should add 5 new sections to every guide content file in src/lib/guides/content/:
- 'How to measure', 'How to adjust', 'Warning signs', 'Variations by rider type', 'Practical recommendation'
- Both EN and NL versions, specific with concrete numbers

When complete, run: tmux-ide task done 044 --proof 'sections added to all content modules; tsc --noEmit passes'

Goal: 09. Mission: guide guideline alignment.