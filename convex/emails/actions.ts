...
export const sendFitReport = action({
  ...
  handler: async (ctx, args): Promise<{ success: boolean; emailId?: string }> => {
    ...
    const { data, error } = await resend.emails.send({
      ...
      to: [recipientEmail],
      subject: "Your BikeFit AI Recommendations",
...
<head>
  ...
  <title>Your BikeFit AI Recommendations</title>
</head>
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #f3f4f6;">
  <div style="max-width: 600px; margin: 0 auto; padding: 40px 20px;">
    <!-- Header -->
    <div style="text-align: center; margin-bottom: 32px;">
      <h1 style="color: #2563eb; font-size: 28px; margin: 0;">BikeFit AI</h1>
